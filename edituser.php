...
if ($form_action && Filter::checkCsrf()) {
    switch ($form_action) {
        case 'update':
            if ($form_username !== Auth::user()
                                       ->getUserName()
                && User::findByIdentifier($form_username)
            ) {
            ...
            } elseif ($form_email !== Auth::user()
                                          ->getEmail()
                      && User::findByIdentifier($form_email)
            ) {
                ...
                if ($form_username !== Auth::user()
                    ...
                    Log::addAuthenticationLog('User ' . Auth::user()
                                                            ->getUserName() . ' renamed to ' . $form_username);
                    Auth::user()
...
<div id="edituser-page">
    ...
    <form name="editform" method="post" action="?" onsubmit="return checkform(this);">
        ...
        <div id="edituser-table">
            ...
            <div class="value">
                <input type="text" id="form_username" name="form_username"
                       value="<?php echo Filter::escapeHtml(Auth::user()
                                                                ->getUserName()); ?>" autofocus>
            ...
            <div class="value">
                <input type="text" id="form_realname" name="form_realname"
                       value="<?php echo Filter::escapeHtml(Auth::user()
                                                                ->getRealName()); ?>">
            ...
            <div class="value">
                <input type="email" id="form_email" name="form_email" value="<?php echo Filter::escapeHtml(Auth::user()
                                                                                                               ->getEmail()); ?>"
                       size="50">
            ...
            <?php if (Site::getPreference('ALLOW_USER_THEMES')): ?>
                ...
                <div class="value">
                    <select id="form_theme" name="form_theme">
                        <option value="">
                            <?php echo Filter::escapeHtml(/* I18N: default option in list of themes */
                                I18N::translate('<default theme>')); ?>
                        </option>
                        <?php foreach (Theme::themeNames() as $theme_id => $theme_name): ?>
                            <option value="<?php echo $theme_id; ?>" <?php echo $theme_id === Auth::user()
                                                                                                  ->getPreference('theme')
                                ? 'selected' : ''; ?>>
                                <?php echo $theme_name; ?>
                            </option>
        ...
        <?php if (!Auth::user()
            ...
            <a href="#"
               onclick="if (confirm('<?php echo I18N::translate('Are you sure you want to delete “%s”?', Filter::escapeJs(Auth::user()
                                                                                                                              ->getUserName())); ?>')) {jQuery('#form_action').val('delete'); document.editform.submit(); }">
