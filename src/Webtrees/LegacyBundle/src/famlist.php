...
switch ($show_marnm) {
    ...
    case 'yes':
        Auth::user()
            ->setPreference(WT_SCRIPT_NAME . '_show_marnm', $show_marnm);
        break;
    default:
        $show_marnm = Auth::user()
                          ->getPreference(WT_SCRIPT_NAME . '_show_marnm');
}
...
$list = array();
foreach (WT_Query_Name::surnameAlpha($show_marnm === 'yes', true, WT_GED_ID) as $letter => $count) {
    switch ($letter) {
        case '@':
...
if ($show === 'indi' || $show === 'surn') {
    ...
    if ($show === 'surn') {
        ...
        if ($count < Globals::i()->WT_TREE->getPreference('SUBLIST_TRIGGER_I')) {
            ...
            $givn_initials = WT_Query_Name::givenAlpha($surname, $alpha, $show_marnm === 'yes', true, WT_GED_ID);
        ...
        if ($show === 'indi') {
            echo FunctionsPrintLists::i()->format_fam_table(WT_Query_Name::families($surname, $alpha, $falpha, $show_marnm === 'yes', WT_GED_ID));
        }
    }
}
