    ...
    <table>
        <tr>
            ...
            foreach ($controller->people as $p => $indi) {
                ...
                <td class="person<?php echo $col; ?>" style="padding: 5px;">
                    ...
                    if ($indi && $indi->canShow()) {
                        ...
                        <input type="hidden" name="pids[<?php echo $p; ?>]"
                               value="<?php echo Filter::escapeHtml($pid); ?>">
                        ...
                        <?php if (!empty($controller->birthyears[$pid])) { ?>
                            <span class="details1"><br>
                                <?php echo /* I18N: an age indicator, which can be dragged around the screen */
                                I18N::translate('Show an age cursor?'); ?>
                                <input type="checkbox" name="agebar<?php echo $p; ?>" value="ON"
                                       onclick="jQuery('#agebox<?php echo $p; ?>').toggle();">
							</span>
                        <?php } ?>
                        <br>
                    <?php
            ...
            <td class="list_value" style="padding: 5px;" valign="top">
                ...
                <div style="text-align: center;"><input type="submit" value="<?php echo I18N::translate('Add'); ?>">
                </div>
            </td>
            <?php
            if (count($controller->people) > 0) {
                ...
                <td class="list_value" style="padding: 5px;">
                    <a href="<?php echo WT_SCRIPT_NAME . "?" . $controller->pidlinks . "scale=" . ($controller->scale + $scalemod); ?>&amp;ged=<?php echo WT_GEDURL; ?>"
                       class="icon-zoomin" title="<?php echo I18N::translate('Zoom in'); ?>"></a><br>
                    <a href="<?php echo WT_SCRIPT_NAME . "?" . $controller->pidlinks . "scale=" . ($controller->scale - $scalemod); ?>&amp;ged=<?php echo WT_GEDURL; ?>"
                       class="icon-zoomout" title="<?php echo I18N::translate('Zoom out'); ?>"></a><br>
                    <input type="button" value="<?php echo I18N::translate('Clear chart'); ?>"
                           onclick="window.location = 'timeline.php?ged=<?php echo WT_GEDURL; ?>';">
                </td>
            <?php } ?>
        </tr>
    </table>
...
if (count($controller->people) > 0) {
    ...
    <div id="timeline_chart">
        ...
        <div id="scale<?php echo $controller->baseyear; ?>"
             style="position:absolute; <?php echo(Globals::i()->TEXT_DIRECTION == "ltr" ? "left: $basexoffset"
                 : "right: $basexoffset"); ?>px; top: <?php echo($baseyoffset - 5); ?>px; font-size: 7pt; text-align: <?php echo(Globals::i()->TEXT_DIRECTION == "ltr"
                 ? "left" : "right"); ?>;">
        ...
        for ($i = $controller->baseyear + 1; $i < $controller->topyear; $i++) {
            if ($i % $mod == 0) {
                echo "<div id=\"scale$i\" style=\"position:absolute; " . (Globals::i()->TEXT_DIRECTION == "ltr" ? "left: $basexoffset"
                        : "right: $basexoffset") . "px; top:" . ($baseyoffset + (($i - $controller->baseyear) * $controller->scale) - $controller->scale / 2) . "px; font-size: 7pt; text-align:" . (Globals::i()->TEXT_DIRECTION === "ltr"
                        ? "left" : "right") . ";\">";
                echo $i . '—';
                echo '</div>';
            }
        }
        echo "<div id=\"scale{$controller->topyear}\" style=\"position:absolute; " . (Globals::i()->TEXT_DIRECTION == "ltr"
                ? "left: $basexoffset"
                : "right: $basexoffset") . "px; top:" . ($baseyoffset + (($controller->topyear - $controller->baseyear) * $controller->scale)) . "px; font-size: 7pt; text-align:" . (Globals::i()->TEXT_DIRECTION == "ltr"
                ? "left" : "right") . ";\">";
        ...
        foreach ($controller->people as $p => $indi) {
            ...
            $col        = $p % 6;
            ?>
            <div id="agebox<?php echo $p; ?>"
                 style="cursor:move; position:absolute; <?php echo(Globals::i()->TEXT_DIRECTION == "ltr"
                     ? "left: " . ($basexoffset + 20)
                     : "right: " . ($basexoffset + 20)); ?>px; top:<?php echo $ageyoffset; ?>px; height:<?php echo $controller->bheight; ?>px; display:none;"
                 onmousedown="ageCursorMouseDown(this, <?php echo $p; ?>);">
                                ...
                                <table class="person<?php echo $col; ?>" style="cursor: hand;">
									...
									<?php echo $tyear; ?>
                                        ...
                                        <td valign="top" width="130">(<?php echo I18N::translate('Age'); ?>
                                            <span id="ageform<?php echo $p; ?>"
                                                  class="field"><?php echo $tage; ?></span>)
                                        </td>
                                    </tr>
                                </table>
                            <?php } ?>
