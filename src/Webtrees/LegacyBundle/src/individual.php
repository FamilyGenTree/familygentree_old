...
if ($controller->record->canShow()) {
    ...
    if ($bdate->isOK() && !$controller->record->isDead()) {
        ...
        echo WT_Gedcom_Tag::getLabelValue('AGE', FunctionsDate::i()->get_age_at_event(Date::GetAgeGedcom($bdate), true), $controller->record, 'span');
    } elseif ($bdate->isOK() && $ddate->isOK()) {
        // If dead, show age at death
        echo WT_Gedcom_Tag::getLabelValue('AGE', FunctionsDate::i()->get_age_at_event(Date::GetAgeGedcom($bdate, $ddate), false), $controller->record, 'span');
    }
    echo '</span>';
