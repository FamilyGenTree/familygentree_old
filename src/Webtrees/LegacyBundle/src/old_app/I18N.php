...
class I18N
{
    ...
    {
        self::$translation_adapter->getAdapter()
                                  ->addTranslation(array('content' => $translation));
    }
    ...
    public static function number($n, $precision = 0)
    {
        ...
        $n = Zend_Locale_Format::toNumber($n, array(
            'locale'    => WT_LOCALE,
            'precision' => $precision
        ));
        $n = self::digits($n);
    ...
    {
        $args    = func_get_args();
        $args[0] = self::$translation_adapter->getAdapter()
                                             ->_($args[0]);
    ...
    public static function translate_c(/* var_args */)
    {
        ...
        $msgtxt = self::$translation_adapter->getAdapter()
                                            ->_($msgid);
        if ($msgtxt == $msgid) {
            $msgtxt = $args[1];
        }
    ...
    {
        return self::$translation_adapter->getAdapter()
                                         ->_($string);
    }
    ...
    {
        $args   = func_get_args();
        $string = self::$translation_adapter->getAdapter()
     ...
     * NOTE: this function is not yet complete - eventually it will replace FunctionsDate::i()->get_age_at_event()
     *
     * @param $string
     *
     * @return string
    ...
    {
        $language_tag = str_replace(array(
                                        '_',
                                        '@'
                                    ), '-', $locale);
    ...
    public static function datatablesI18N(array $lengths = null)
    {
        ...
        foreach ($lengths as $length) {
            $length_menu .=
                '<option value="' . $length . '">' .
                ($length == -1 ? /* I18N: listbox option, e.g. “10,25,50,100,all” */
                    self::translate('All') : self::number($length)) .
                '</option>';
        }
        $length_menu = '<select>' . $length_menu . '</select>';
        $length_menu = /* I18N: Display %s [records per page], %s is a placeholder for listbox containing numeric options */
            self::translate('Display %s', $length_menu);
        ...
        return
            '"language": {' .
            ' "paginate": {' .
            '  "first":    "' . /* I18N: button label, first page    */
            self::translate('first') . '",' .
            '  "last":     "' . /* I18N: button label, last page     */
            self::translate('last') . '",' .
            '  "next":     "' . /* I18N: button label, next page     */
            self::translate('next') . '",' .
            '  "previous": "' . /* I18N: button label, previous page */
