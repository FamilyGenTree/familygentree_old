...
class SearchController extends PageController
{
    ...
    function __construct()
    {
        ...
        if ($this->firstname || $this->lastname || $this->place) {
            $this->myquery = Filter::escapeHtml(implode(' ', array(
                $this->firstname,
                $this->lastname,
                $this->place
            )));
    ...
    private function searchAndReplace()
    {
        ...
        $name_tags     = array_unique(array_merge(Constants::$STANDARD_NAME_FACTS, $adv_name_tags));
        $name_tags[]   = '_MARNM';
                ...
                foreach ($this->search_trees as $search_tree) {
                    ...
                    foreach ($this->myindilist as $individual) {
                        if ($individual->getTree()
                                       ->getTreeId() === $search_tree->getTreeId()
                        ) {
                            $datalist[] = $individual;
                        }
                    }
