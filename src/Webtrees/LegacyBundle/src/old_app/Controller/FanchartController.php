...
class FanchartController extends ChartController
{
    ...
    {
        return array(
            2 => /* I18N: layout option for the fan chart */
                I18N::translate('half circle'),
            3 => /* I18N: layout option for the fan chart */
                I18N::translate('three-quarter circle'),
            4 => /* I18N: layout option for the fan chart */
    ...
    public function generateFanChart($what)
    {
        ...
        switch ($what) {
            case 'html':
                return $html . $imagemap . '<div id="fan_chart_img"><img src="' . WT_SCRIPT_NAME . '?rootid=' . $this->root->getXref() . '&amp;fan_style=' . $this->fan_style . '&amp;generations=' . $this->generations . '&amp;fan_width=' . $this->fan_width . '&amp;img=1" width="' . $fanw . '" height="' . $fanh . '" alt="' . I18N::translate('Fan chart of %s', strip_tags($person->getFullName())) . '" usemap="#fanmap"></div>';

            case 'png':
                ImageStringUp($image, 1, $fanw - 10, $fanh / 3, Config::get(Config::BASE_URL), $color);
                ob_start();
                ImagePng($image);
                ImageDestroy($image);
