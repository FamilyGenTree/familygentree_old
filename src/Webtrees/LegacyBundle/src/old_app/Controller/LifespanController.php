...
class LifespanController extends PageController
{
    ...
    function __construct()
    {
        ...
        if (isset ($this->people[0])) {
            ...
            foreach ($this->people as $value) {
                ...
                $this->timelineMaxYear = max($this->timelineMaxYear, $ddate->gregorianYear() ? $ddate->gregorianYear()
                    : date('Y'));
            }
    ...
    public function fillTimeline($ar, $top)
    {
        ...
        foreach ($ar as $value) {
            ...
            $deathReal = $value->getDeathDate()
                               ->isOK();
            $birthReal = $value->getBirthDate()
                               ->isOK();
            ...
            foreach ($unparsedEvents as $val) {
                ...
                if (!empty($date)) {
                    ...
                    if (isset($eventinformation[$evntwdth])) {
                        $eventinformation[$evntwdth] .= '<br>' . $trans . '<br>' . strip_tags($date->Display()) . ' ' . $val->getPlace()
                                                                                                                            ->getFullName();
                    } else {
                        $eventinformation[$evntwdth] = $fact . '-fact, ' . $trans . '<br>' . strip_tags($date->Display()) . ' ' . $val->getPlace()
                                                                                                                                      ->getFullName();
                    }
                }
            }
