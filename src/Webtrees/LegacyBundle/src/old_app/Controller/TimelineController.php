...
class TimelineController extends PageController
{
    ...
    function __construct()
    {
        ...
        if (count($this->pids) == 0) {
            $this->pids[] = $this->getSignificantIndividual()
                                 ->getXref();
        }
    ...
    function print_time_fact(Fact $event)
    {
        ...
        echo "<div id=\"fact$factcount\" style=\"position:absolute; " . (Globals::i()->TEXT_DIRECTION == "ltr"
                ? "left: " . ($xoffset)
        ...
        if ($col === false) {
            ...
            $col = array_search($event->getParent()
                                      ->getHusband(), $this->people);
        }
        if ($col === false) {
        ...
        if ($event->getParent() instanceof Family) {
            echo ' — <a href="', $event->getParent()
                                       ->getHtmlUrl(), '">', $event->getParent()
                                                                   ->getFullName(), '</a>';
        }
        echo '</td></tr></table>';
        echo '</div>';
    ...
    {
        if ($this->pids) {
            return Individual::getInstance($this->pids[0]);
        } else {
