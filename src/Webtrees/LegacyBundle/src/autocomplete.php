...
switch ($type) {
    case 'ASSO': // Associates of an individuals, whose name contains the search terms
        ...
        $rows =
            Database::i()->prepare(
                ...
                " WHERE (n_full LIKE CONCAT('%', REPLACE(?, ' ', '%'), '%') OR n_surn LIKE CONCAT('%', REPLACE(?, ' ', '%'), '%')) AND i_file=? ORDER BY n_full COLLATE '" . I18N::$collation . "'"
            )
                    ->execute(array(
                                  $term,
                                  $term,
        ...
        foreach ($rows as $row) {
            ...
            if ($person->canShow()) {
                if ($event_jd) {
                    ...
                    $person_birth_jd = $person->getEstimatedBirthDate()
                                              ->minJD();
                    if ($person_birth_jd && $person_birth_jd > $event_jd) {
                        continue;
                    }
                    // Exclude individuals who died before the event.
                    $person_death_jd = $person->getEstimatedDeathDate()
                                              ->MaxJD();
                ...
                if ($event_jd
                    ...
                    $label .= ', <span class="age">(' . I18N::translate('Age') . ' ' . $person->getBirthDate()
                                                                                              ->MinDate()
                                                                                              ->getAge(false, $event_jd) . ')</span>';
                } else {
    ...
    case 'GIVN': // Given names, that start with the search term
        ...
        echo json_encode(
            Database::i()->prepare(
                ...
                " ORDER BY n_givn COLLATE '" . I18N::$collation . "'"
            )
                    ->execute(array(
                                  $term,
                                  WT_GED_ID
            ...
            if ($person->canShowName()) {
                $data[] = array(
                    ...
                    'label' => str_replace(array(
                                               ...
                                               Constants::UNKNOWN_NN(),
                                               Constants::UNKNOWN_PN()
                                           ), $row->n_full) . ', <i>' . $person->getLifeSpan() . '</i>'
                );
            }
        }
        echo json_encode($data);
    ...
    case 'NOTE': // Notes which contain the search terms
        ...
        // Filter for privacy
        foreach ($rows as $row) {
            $note = Note::getInstance($row->xref, $row->gedcom_id, $row->gedcom);
            if ($note->canShowName()) {
                $data[] = array(
                    'value' => $note->getXref(),
    ...
    case 'PLAC2': // Place names (without hierarchy), that include the search term
        ...
        echo json_encode(
            Database::i()->prepare(
                ...
                " ORDER BY p_place COLLATE '" . I18N::$collation . "'"
            )
                    ->execute(array(
                                  $term,
                                  WT_GED_ID
    ...
    case 'SOUR_TITL': // Source titles, that include the search terms
        ...
        // Filter for privacy
        foreach ($rows as $row) {
            $source = Source::getInstance($row->xref, $row->gedcom_id, $row->gedcom);
    ...
    case 'SURN': // Surnames, that start with the search term
        ...
        echo json_encode(
            Database::i()->prepare(
                ...
                " ORDER BY n_surname COLLATE '" . I18N::$collation . "'"
            )
                    ->execute(array(
                                  $term,
                                  WT_GED_ID
    ...
    case 'IFSRO':
        ...
        // Filter for privacy
        foreach ($rows as $row) {
            $repository = Repository::getInstance($row->xref, $row->gedcom_id, $row->gedcom);
            if ($repository->canShowName()) {
                $data[] = array(
                    'value' => $repository->getXref(),
                    'label' => $repository->getFullName()
                );
            }
        }
        // Fetch all data, regardless of privacy
...
function get_FAM_rows($term)
{
    return
        Database::i()->prepare(
            ...
            " ORDER BY husb_name.n_sort, wife_name.n_sort COLLATE '" . I18N::$collation . "'"
        )
                ->execute(array(
                              $term,
                              WT_GED_ID
...
function get_INDI_rows($term)
{
    return
        Database::i()->prepare(
            ...
            " FROM `##individuals`" .
            " JOIN `##name` ON (i_id=n_id AND i_file=n_file)" .
            " WHERE n_full LIKE CONCAT('%', REPLACE(?, ' ', '%'), '%') AND i_file=? ORDER BY n_full COLLATE '" . I18N::$collation . "'"
        )
                ->execute(array(
                              $term,
                              WT_GED_ID
...
function get_NOTE_rows($term)
{
    return
        Database::i()->prepare(
            ...
            " ORDER BY n_full COLLATE '" . I18N::$collation . "'"
        )
                ->execute(array(
                              $term,
                              WT_GED_ID
