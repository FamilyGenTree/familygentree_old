...
switch ($type) {
    ...
    case "specialchar":
        ...
        if (!$language_filter) {
            $language_filter = Auth::user()
                                   ->getPreference('default_language_filter');
        } else {
            Auth::user()
...
if ($type == "facts") {
	...
	function initPickFact() {
		var n,i,j,tmp,preselectedDefaultTags="\x01<?php foreach ($preselDefault as $p) {
        echo addslashes($p), '\\x01';
    } ?>";
