...
$earliest = Database::i()->prepare("SELECT DATE(MIN(change_time)) FROM `##change`")
                    ->execute(array())
                    ->fetchOne();
$latest   = Database::i()->prepare("SELECT DATE(MAX(change_time)) FROM `##change`")
                    ->execute(array())
                    ->fetchOne();
...
switch ($action) {
    ...
    case 'export':
        ...
        $rows = Database::i()->prepare($SELECT1 . $WHERE . ' ORDER BY change_id')
                        ->execute($args)
                        ->fetchAll();
        foreach ($rows as $row) {
    ...
    case 'load_json':
        ...
        $search = $search['value'];
        Auth::user()
            ->setPreference('admin_site_change_page_size', $length);
        if ($length > 0) {
...
$controller
    ...
    ->addInlineJavascript('
		jQuery(".table-site-changes").dataTable( {
			processing: true,
			serverSide: true,
			ajax: "' . Config::get(Config::BASE_URL) . WT_SCRIPT_NAME . '?action=load_json&from=' . $from . '&to=' . $to . '&type=' . $type . '&oldged=' . rawurlencode($oldged) . '&newged=' . rawurlencode($newged) . '&xref=' . rawurlencode($xref) . '&user=' . rawurlencode($user) . '&gedc=' . rawurlencode($gedc) . '",
			...
			pageLength: ' . Auth::user()
                                ->getPreference('admin_site_change_page_size', 10) . ',
			columns: [
			/* Timestamp   */ { },
			/* Status      */ { },
			/* Record      */ { },
...
<form class="form" name="logs">
    ...
    <div class="row">
        <div class="form-group col-xs-6 col-md-3">
            <label for="from">
                <?php echo /* I18N: label for the start of a date range (from x to y) */
                I18N::translate('From'); ?>
            </label>

            <div class="input-group date">
                <input type="text" autocomplete="off" class="form-control" id="from" name="from"
                       value="<?php echo Filter::escapeHtml($from); ?>">
        ...
        <div class="form-group col-xs-6 col-md-3">
            <label for="to">
                <?php /* I18N: label for the end of a date range (from x to y) */
                echo I18N::translate('To'); ?>
            </label>

            <div class="input-group date">
                <input type="text" autocomplete="off" class="form-control" id="to" name="to"
                       value="<?php echo Filter::escapeHtml($to); ?>">
                <span class="input-group-addon"><span class="fa fa-calendar"></span></span>
            </div>
        </div>
        ...
        <div class="form-group col-xs-6 col-md-3">
            ...
            <input class="form-control" type="text" id="xref" name="xref"
                   value="<?php echo Filter::escapeHtml($xref); ?>">
        </div>
    </div>
    ...
    <div class="row">
        <div class="form-group col-xs-6 col-md-3">
            ...
            <input class="form-control" type="text" id="oldged" name="oldged"
                   value="<?php echo Filter::escapeHtml($oldged); ?>">
        </div>
        ...
        <div class="form-group col-xs-6 col-md-3">
            ...
            <input class="form-control" type="text" id="newged" name="newged"
                   value="<?php echo Filter::escapeHtml($newged); ?>">
        </div>
    ...
    <div class="row text-center">
        ...
        <button type="submit" class="btn btn-primary"
                onclick="document.logs.action.value='export';return true;" <?php echo $action === 'show' ? ''
            : 'disabled'; ?>>
        ...
        <button type="submit" class="btn btn-primary"
                onclick="if (confirm('<?php echo I18N::translate('Permanently delete these records?'); ?>')) {document.changes.action.value='delete'; return true;} else {return false;}" <?php echo $action === 'show'
            ? '' : 'disabled'; ?>>
