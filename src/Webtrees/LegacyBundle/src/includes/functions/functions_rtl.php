...
class FunctionsRtl
{
     ...
     */
    protected function stripLRMRLM($inputText)
    {
    ...
    function spanLTRRTL($inputText, $direction = 'BOTH', $class = '')
    {
        ...
        $workingText = $this->stripLRMRLM($workingText); // Get rid of any existing UTF8 control codes
        ...
        while ($workingText != '') {
            $charArray     = $this->getChar($workingText, 0); // Get the next ASCII or UTF-8 character
            ...
            switch ($currentLetter) {
                ...
                default:
                    ...
                    if ($numberState) {
                        ...
                        $charArray = $this->getChar($workingText . "\n", $offset);
                    ...
                    while (true) {
                        if (I18N::scriptDirection(I18N::textScript($currentLetter)) === 'rtl') {
                            ...
                            while ($tempText != '') {
                                $nextCharArray = $this->getChar($tempText, 0);
                    ...
                    if ($newState != $currentState) {
                        ...
                        $this->finishCurrentSpan($result, false);
        ...
        while (substr($result, 0, $lenStart + 3) == $startRTL . WT_UTF8_LRE) {
            ...
            $textSpan = $this->stripLRMRLM(substr($result, $lenStart + 3, $spanEnd - $lenStart - 3));
        ...
        $result = str_replace(array(
                                  $endRTL . $startLTR . '-' . $endLTR . $startRTL,
                                  $endRTL . $startLTR . '-' . $endLTR . $startRTL
                              ), array(
                                  '-',
                                  '+'
     ...
     * @return string
     */
    protected function starredName($textSpan, $direction)
    {
     ...
     * @return array
     */
    protected function getChar($text, $offset)
    {
     ...
     * @param string $result
     */
    protected function breakCurrentSpan(&$result)
    {
     ...
     * @param string $result
     */
    protected function beginCurrentSpan(&$result)
    {
     ...
     */
    protected function finishCurrentSpan(&$result, $theEnd = false)
    {
        ...
        if ($currentState == 'LTR') {
            ...
            while (Globals::i()->TEXT_DIRECTION == 'rtl') {
                ...
                while (true) {
                    ...
                    $temp = $this->stripLRMRLM(substr($textSpan, $posLeftParen)); // Get rid of UTF8 control codes
            ...
            $textSpan = $this->starredName($textSpan, 'LTR'); // Wrap starred name in <u> and </u> tags
            while (true) {
        ...
        if ($currentState == 'RTL') {
            ...
            if ($countLeadingSpaces != 0) {
                ...
                $textSpan  = str_pad($textSpan, $newLength, ' ', (Globals::i()->TEXT_DIRECTION == 'rtl' ? STR_PAD_LEFT
                    : STR_PAD_RIGHT));
            }
            ...
            // We're done: finish the span
            $textSpan = $this->starredName($textSpan, 'RTL'); // Wrap starred name in <u> and </u> tags
