...
// Print an edit control for a Yes/No field
    ...
    {
        return '<input type="checkbox" name="' . $name . '" value="1" ' . ($is_checked ? 'checked '
            : '') . $extra . '>';
    }
    ...
    function edit_field_access_level($name, $selected = '', $extra = '')
    {
        ...
        return $this->select_edit_control($name, $ACCESS_LEVEL, null, $selected, $extra);
    }
    ...
    function edit_field_resn($name, $selected = '', $extra = '')
    {
        ...
        return $this->select_edit_control($name, $RESN, null, $selected, $extra);
    }
    ...
    function edit_field_contact($name, $selected = '', $extra = '')
    {
        ...
        return $this->select_edit_control($name, $CONTACT_METHODS, null, $selected, $extra);
    }
    ...
    {
        return $this->select_edit_control($name, I18N::installed_languages(), null, $selected, $extra);
    }
    ...
    function edit_field_integers($name, $selected = '', $min, $max, $extra = '')
    {
        ...
        return $this->select_edit_control($name, $array, null, $selected, $extra);
    }
    ...
    function edit_field_username($name, $selected = '', $extra = '')
    {
        ...
        return $this->select_edit_control($name, $all_users, '-', $selected, $extra);
    }
    ...
    {
        return $this->select_edit_control($name, WT_Gedcom_Code_Adop::getValues($individual), null, $selected, $extra);
    }
    ...
    {
        return $this->select_edit_control($name, WT_Gedcom_Code_Pedi::getValues($individual), '', $selected, $extra);
    }
    ...
    {
        return $this->select_edit_control($name, WT_Gedcom_Code_Name::getValues($individual), '', $selected, $extra);
    }
    ...
    function edit_field_rela($name, $selected = '', $extra = '')
    {
        ...
        return $this->select_edit_control($name, $rela_codes, '', $selected, $extra);
    }
    ...
    {
        global $tags, $main_fact, $xref, $bdm, $action;
...
// help link
        ...
        if (in_array($fact, Constants::$EMPTY_FACTS) && ($value == '' || $value == 'Y' || $value == 'y')) {
            ...
            echo $this->select_edit_control($element_name, WT_Gedcom_Code_Temp::templeNames(), I18N::translate('No temple - living ordinance'), $value);
        } else if ($fact === 'ADOP') {
            echo $this->edit_field_adop($element_name, $value, '', $person);
        } else if ($fact === 'PEDI') {
            echo $this->edit_field_pedi($element_name, $value, '', $person);
        } else if ($fact === 'STAT') {
            echo $this->select_edit_control($element_name, WT_Gedcom_Code_Stat::statusNames($upperlevel), '', $value);
        } else if ($fact === 'RELA') {
            echo $this->edit_field_rela($element_name, strtolower($value));
        } else if ($fact === 'QUAY') {
            echo $this->select_edit_control($element_name, WT_Gedcom_Code_Quay::getValues(), '', $value);
        } else if ($fact === '_WT_USER') {
            echo $this->edit_field_username($element_name, $value);
        } else if ($fact === 'RESN') {
            echo $this->edit_field_resn($element_name, $value);
        } else if ($fact === '_PRIM') {
        ...
        if ($fact == 'TYPE' && $level == 2 && $tags[0] == 'MARR') {
            ...
            echo $this->edit_field_name_type($element_name, $value, $onchange, $person);
            echo '<script>';
            echo "document.getElementById('", $element_id, "').style.display='none';";
            echo '</script>';
        }
        ...
        switch ($fact) {
            case 'DATE':
                echo $this->print_calendar_popup($element_id);
    ...
    function print_add_layer($tag, $level = 2)
    {
        switch ($tag) {
            case 'SOUR':
                ...
                // 2 SOUR
                $this->add_simple_tag($level . ' SOUR @');
                // 3 PAGE
                $this->add_simple_tag(($level + 1) . ' PAGE');
                // 3 DATA
                // 4 TEXT
                $this->add_simple_tag(($level + 2) . ' TEXT');
                if (Globals::i()->WT_TREE->getPreference('FULL_SOURCES')) {
                    // 4 DATE
                    $this->add_simple_tag(($level + 2) . ' DATE', '', WT_Gedcom_Tag::getLabel('DATA:DATE'));
                    // 3 QUAY
                    $this->add_simple_tag(($level + 1) . ' QUAY');
                }
                // 3 OBJE
                $this->add_simple_tag(($level + 1) . ' OBJE');
                // 3 SHARED_NOTE
                $this->add_simple_tag(($level + 1) . ' SHARED_NOTE');
                echo '</table></div>';
                break;
            ...
            case 'ASSO2':
                ...
                echo '<table class="facts_table">';
                // 2 ASSO
                $this->add_simple_tag($level . ' ASSO @');
                // 3 RELA
                $this->add_simple_tag(($level + 1) . ' RELA');
                // 3 NOTE
                $this->add_simple_tag(($level + 1) . ' NOTE');
                // 3 SHARED_NOTE
                $this->add_simple_tag(($level + 1) . ' SHARED_NOTE');
                echo '</table></div>';
                break;
            ...
            case 'NOTE':
                ...
                echo '<table class="facts_table">';
                // 2 NOTE
                $this->add_simple_tag($level . ' NOTE');
                echo '</table></div>';
                break;
            ...
            case 'SHARED_NOTE':
                ...
                echo '<table class="facts_table">';
                // 2 SHARED NOTE
                $this->add_simple_tag($level . ' SHARED_NOTE');
                echo '</table></div>';
                break;
            ...
            case 'OBJE':
                if (Globals::i()->WT_TREE->getPreference('MEDIA_UPLOAD') >= WT_USER_ACCESS_LEVEL) {
                    ...
                    echo '<table class="facts_table">';
                    $this->add_simple_tag($level . ' OBJE');
                    echo '</table></div>';
                }
                break;
            ...
            case 'RESN':
                ...
                echo '<table class="facts_table">';
                // 2 RESN
                $this->add_simple_tag($level . ' RESN');
                echo '</table></div>';
                break;
        }
    }
    ...
    function addSimpleTags($fact)
    {
        ...
        if ($fact == 'MARR') {
            $this->add_simple_tag("0 {$fact} Y");
        } else {
            $this->add_simple_tag("0 {$fact}");
        }

        if (!in_array($fact, Constants::$NON_DATE_FACTS)) {
            $this->add_simple_tag("0 DATE", $fact, WT_Gedcom_Tag::getLabel("{$fact}:DATE"));
        }

        if (!in_array($fact, Constants::$NON_PLAC_FACTS)) {
            $this->add_simple_tag("0 PLAC", $fact, WT_Gedcom_Tag::getLabel("{$fact}:PLAC"));

            if (preg_match_all('/(' . WT_REGEX_TAG . ')/', Globals::i()->WT_TREE->getPreference('ADVANCED_PLAC_FACTS'), $match)) {
                foreach ($match[1] as $tag) {
                    $this->add_simple_tag("0 {$tag}", $fact, WT_Gedcom_Tag::getLabel("{$fact}:PLAC:{$tag}"));
                }
            }
            $this->add_simple_tag("0 MAP", $fact);
            $this->add_simple_tag("0 LATI", $fact);
            $this->add_simple_tag("0 LONG", $fact);
        }
    }
    ...
    function addNewFact($fact)
    {
        ...
        if ($DATE || $PLAC || $FACT && $FACT != 'Y') {
            ...
            if (Filter::postBool("SOUR_{$fact}")) {
                return $this->updateSOUR($gedrec, 2);
            } else {
                return $gedrec;
            }
        } elseif ($FACT == 'Y') {
            if (Filter::postBool("SOUR_{$fact}")) {
                return $this->updateSOUR("\n1 {$fact} Y", 2);
            } else {
                return "\n1 {$fact} Y";
            }
        } else {
     ...
     * were produced by the FunctionsEdit::splitSOUR() function.
     * See the $this->handle_updates() function for details.
     *
    ...
    function updateSOUR($inputRec, $levelOverride = 'no')
    {
        ...
        $myRecord = $this->handle_updates($inputRec, $levelOverride); // Now do the update
     ...
     * The edit_interface and $this->add_simple_tag function produce the following
    ...
    function create_add_form($fact)
    {
        ...
        if (substr($fact, 0, 5) == 'MARR_') {
            $tags[0] = 'MARR';
            $this->add_simple_tag('1 MARR');
            $this->insert_missing_subtags($fact);
        } else {
            $tags[0] = $fact;
            if ($fact == '_UID') {
            ...
            if (in_array($fact, Constants::$EMPTY_FACTS)) {
                $this->add_simple_tag('1 ' . $fact . ' Y');
            } else {
                $this->add_simple_tag('1 ' . $fact);
            }
            $this->insert_missing_subtags($tags[0]);
            //-- handle the special SOURce case for level 1 sources [ 1759246 ]
            if ($fact == 'SOUR') {
                $this->add_simple_tag('2 PAGE');
                $this->add_simple_tag('3 TEXT');
                if (Globals::i()->WT_TREE->getPreference('FULL_SOURCES')) {
                    $this->add_simple_tag('3 DATE', '', WT_Gedcom_Tag::getLabel('DATA:DATE'));
                    $this->add_simple_tag('2 QUAY');
                }
            }
        }
    }
    ...
    function create_edit_form(GedcomRecord $record, Fact $fact)
    {
        ...
        while (true) {
            ...
            if ($type != "DATA" && $type != "CONT") {
                ...
                if ($inSource && $type == "DATE") {
                    $this->add_simple_tag($subrecord, '', WT_Gedcom_Tag::getLabel($label, $person));
                } elseif (!$inSource && $type == "DATE") {
                    $this->add_simple_tag($subrecord, $level1type, WT_Gedcom_Tag::getLabel($label, $person));
                    $add_date = false;
                } elseif ($type == 'STAT') {
                    $this->add_simple_tag($subrecord, $level1type, WT_Gedcom_Tag::getLabel($label, $person));
                } elseif ($level0type == 'REPO') {
                    $repo = Repository::getInstance($pid);
                    $this->add_simple_tag($subrecord, $level0type, WT_Gedcom_Tag::getLabel($label, $repo));
                } else {
                    $this->add_simple_tag($subrecord, $level0type, WT_Gedcom_Tag::getLabel($label, $person));
                }
            }
                ...
                foreach ($expected_subtags[$type] as $subtag) {
                    if (!in_array($subtag, $subtags)) {
                        if (!$inSource || $subtag != "DATA") {
                            $this->add_simple_tag(($level + 1) . ' ' . $subtag, '', WT_Gedcom_Tag::getLabel("{$label}:{$subtag}"));
                        }
                        if (!empty($expected_subtags[$subtag])) {
                            foreach ($expected_subtags[$subtag] as $subsubtag) {
                                $this->add_simple_tag(($level + 2) . ' ' . $subsubtag, '', WT_Gedcom_Tag::getLabel("{$label}:{$subtag}:{$subsubtag}"));
                            }
                        }
                    }
                }
            ...
            if ($level == 2 && $type == 'DATE' && in_array($level1type, Constants::$DATE_AND_TIME) && !in_array('TIME', $subtags)) {
                $this->add_simple_tag("3 TIME"); // TIME is NOT a valid 5.5.1 tag
            }
            if ($level == 2 && $type == 'STAT' && WT_Gedcom_Code_Temp::isTagLDS($level1type) && !in_array('DATE', $subtags)) {
                $this->add_simple_tag("3 DATE", '', WT_Gedcom_Tag::getLabel('STAT:DATE'));
            }
        ...
        if ($level1type != '_PRIM') {
            $this->insert_missing_subtags($level1type, $add_date);
        }
    ...
    function insert_missing_subtags($level1tag, $add_date = false)
    {
        ...
        foreach (Constants::$LEVEL2_TAGS as $key => $value) {
            if ($key == 'DATE' && in_array($level1tag, Constants::$NON_DATE_FACTS) || $key == 'PLAC' && in_array($level1tag, Constants::$NON_PLAC_FACTS)) {
                continue;
            }
            if (in_array($level1tag, $value) && !in_array($key, $tags)) {
                if ($key == 'TYPE') {
                    $this->add_simple_tag('2 TYPE ' . $type_val, $level1tag);
                } elseif ($level1tag == '_TODO' && $key == 'DATE') {
                    $this->add_simple_tag('2 ' . $key . ' ' . strtoupper(date('d M Y')), $level1tag);
                } elseif ($level1tag == '_TODO' && $key == '_WT_USER') {
                    $this->add_simple_tag('2 ' . $key . ' ' . Auth::user()
                                                           ->getUserName(), $level1tag);
                } else if ($level1tag == 'TITL' && strstr(Globals::i()->WT_TREE->getPreference('ADVANCED_NAME_FACTS'), $key) !== false) {
                    $this->add_simple_tag('2 ' . $key, $level1tag);
                ...
                switch ($key) {
                    case 'PLAC':
                        if (preg_match_all('/(' . WT_REGEX_TAG . ')/', Globals::i()->WT_TREE->getPreference('ADVANCED_PLAC_FACTS'), $match)) {
                            foreach ($match[1] as $tag) {
                                $this->add_simple_tag("3 $tag", '', WT_Gedcom_Tag::getLabel("{$level1tag}:PLAC:{$tag}"));
                            }
                        }
                        $this->add_simple_tag('3 MAP');
                        $this->add_simple_tag('4 LATI');
                        $this->add_simple_tag('4 LONG');
                        break;
                    case 'FILE':
                        $this->add_simple_tag('3 FORM');
                        break;
                    case 'EVEN':
                        $this->add_simple_tag('3 DATE');
                        $this->add_simple_tag('3 PLAC');
                        break;
                    case 'STAT':
                    ...
                    case 'DATE':
                        ...
                        if (in_array($level1tag, Constants::$DATE_AND_TIME)) {
                            $this->add_simple_tag('3 TIME');
                        }
                        break;
                    case 'HUSB':
                    case 'WIFE':
                        $this->add_simple_tag('3 AGE');
                        break;
                    case 'FAMC':
                        if ($level1tag === 'ADOP') {
                            $this->add_simple_tag('3 ADOP BOTH');
                        }
                        break;
                }
            } elseif ($key === 'DATE' && $add_date) {
                $this->add_simple_tag('2 DATE', $level1tag, WT_Gedcom_Tag::getLabel("{$level1tag}:DATE"));
            }
        }
        ...
        if (substr($level1tag, 0, 1) == '_' && $level1tag != '_UID' && $level1tag != '_TODO') {
            foreach (array(
                ...
                if (!in_array($tag, $tags)) {
                    $this->add_simple_tag("2 {$tag}");
                    if ($tag === 'PLAC') {
                        if (preg_match_all('/(' . WT_REGEX_TAG . ')/', Globals::i()->WT_TREE->getPreference('ADVANCED_PLAC_FACTS'), $match)) {
                            foreach ($match[1] as $ptag) {
                                $this->add_simple_tag("3 $ptag", '', WT_Gedcom_Tag::getLabel("{$level1tag}:PLAC:{$ptag}"));
                            }
                        }
                        $this->add_simple_tag('3 MAP');
                        $this->add_simple_tag('4 LATI');
