...
class FunctionsPrintLists
{
	...
	function format_indi_table($datalist, $option = '')
	{
		...
		$controller
			...
			jQuery("#' . $table_id . '").dataTable( {
				...
				columns: [
					...
					/*  4 sosa      */ { dataSort: 5, class: "center", visible: ' . ($option == 'sosa' ? 'true'
									  : 'false') . ' },
					...
					/* 18 CHAN      */ { dataSort: 19, visible: ' . (Globals::i()->WT_TREE->getPreference('SHOW_LAST_CHANGE')
									  ? 'true' : 'false') . ' },
		...
		foreach ($datalist as $key => $person) {
			...
			foreach ($person->getAllNames() as $num => $name) {
				...
				$html .= '<a ' . $title . ' href="' . $person->getHtmlUrl() . '"' . $class . '>' . FunctionsPrint::i()
																												 ->highlight_search_hits($name['full']) . '</a>' . $sex_image . '<br>';
			}
			// Indi parents
		...
		$html .= '
			...
			<div id="indi_list_table-charts_' . $table_id . '" style="display:none">
				<table class="list-charts">
					<tr>
						<td>
							' . FunctionsPrintLists::i()->print_chart_by_decade($birt_by_decade, I18N::translate('Decade of birth')) . '
						</td>
						<td>
							' . FunctionsPrintLists::i()->print_chart_by_decade($deat_by_decade, I18N::translate('Decade of death')) . '
						</td>
					</tr>
					<tr>
						<td colspan="2">
							' . FunctionsPrintLists::i()->print_chart_by_age($deat_by_age, I18N::translate('Age related to death year')) . '
						</td>
					</tr>
				</table>
			</div>
			...
			<table id="' . $table_id . '">
				<thead>
					...
					<tr>
						...
						<th><i class="icon-children" title="' . I18N::translate('Children') . '"></i></th>
					<th>NCHI</th>
					<th' . (Globals::i()->WT_TREE->getPreference('SHOW_LAST_CHANGE') ? ''
				: '') . '>' . WT_Gedcom_Tag::getLabel('CHAN') . '</th>
		...
		foreach ($datalist as $family) {
			...
			foreach ($husb->getAllNames() as $num => $name) {
				...
				if ($name['type'] != '_MARNM' || $num == $husb->getPrimaryName()) {
					$html .= '<a ' . $title . ' href="' . $family->getHtmlUrl() . '"' . $class . '>' . FunctionsPrint::i()
																													 ->highlight_search_hits($name['full']) . '</a>' . $sex_image . '<br>';
				}
			}
			// Husband parents
			...
			<div id="fam_list_table-charts_' . $table_id . '" style="display:none">
				<table class="list-charts">
					<tr>
						...
						<td>
							' . FunctionsPrintLists::i()->print_chart_by_decade($marr_by_decade, I18N::translate('Decade of marriage')) . '
						</td>
					</tr>
					<tr>
						<td colspan="2">
							' . FunctionsPrintLists::i()->print_chart_by_age($marr_by_age, I18N::translate('Age in year of marriage')) . '
						</td>
					</tr>
				</table>
			</div>
	...
	function format_sour_table($datalist)
	{
		...
		$controller
			...
			jQuery("#' . $table_id . '").dataTable( {
				...
				columns: [
					...
					/* 11 CHAN      */ { dataSort: 12, visible: ' . (Globals::i()->WT_TREE->getPreference('SHOW_LAST_CHANGE')
									  ? 'true' : 'false') . ' },
		...
		$html .= '<th>#NOTE</th>';
		$html .= '<th' . (Globals::i()->WT_TREE->getPreference('SHOW_LAST_CHANGE') ? ''
				: '') . '>' . WT_Gedcom_Tag::getLabel('CHAN') . '</th>';
		...
		foreach ($datalist as $source) {
			...
			foreach ($source->getAllNames() as $n => $name) {
				...
				if ($n == $source->getPrimaryName()) {
					$html .= '<a class="name2" href="' . $source->getHtmlUrl() . '">' . FunctionsPrint::i()
																									  ->highlight_search_hits($name['full']) . '</a>';
				} else {
					$html .= '<a href="' . $source->getHtmlUrl() . '">' . FunctionsPrint::i()
																						->highlight_search_hits($name['full']) . '</a>';
				}
			}
			$html .= '</td>';
	...
	function format_note_table($datalist)
	{
		...
		$controller
			...
			jQuery("#' . $table_id . '").dataTable({
				...
				columns: [
					...
					/*  9 CHAN      */ { dataSort: 10, visible: ' . (Globals::i()->WT_TREE->getPreference('SHOW_LAST_CHANGE')
									  ? 'true' : 'false') . ' },
		...
		foreach ($datalist as $note) {
			...
			$html .= '<tr' . $class . '>';
			//-- Shared Note name
			$html .= '<td><a class="name2" href="' . $note->getHtmlUrl() . '">' . FunctionsPrint::i()
																								->highlight_search_hits($note->getFullName()) . '</a></td>';
			//-- Linked INDIs
		...
		$controller
			...
			jQuery("#' . $table_id . '").dataTable({
				...
				columns: [
					...
					/* 3 CHAN      */ { dataSort: 4, visible: ' . (Globals::i()->WT_TREE->getPreference('SHOW_LAST_CHANGE')
									  ? 'true' : 'false') . ' },
		...
		foreach ($repositories as $repository) {
			...
			foreach ($repository->getAllNames() as $n => $name) {
				...
				if ($n == $repository->getPrimaryName()) {
					$html .= '<a class="name2" href="' . $repository->getHtmlUrl() . '">' . FunctionsPrint::i()
																										  ->highlight_search_hits($name['full']) . '</a>';
				} else {
					$html .= '<a href="' . $repository->getHtmlUrl() . '">' . FunctionsPrint::i()
																							->highlight_search_hits($name['full']) . '</a>';
				}
			}
			$html .= '</td>';
		...
		$controller
			...
			jQuery("#' . $table_id . '").dataTable({
				...
				columns: [
					...
					/* 8 CHAN      */ { dataSort: 9, visible: ' . (Globals::i()->WT_TREE->getPreference('SHOW_LAST_CHANGE')
									  ? 'true' : 'false') . ' },
		...
		foreach ($surnames as $surn => $surns) {
			foreach ($surns as $spfxsurn => $indis) {
				$cloud->appendTag(array(
									  'title'  => $totals
										  ? I18N::translate('%1$s (%2$d)', '<span dir="auto">' . $spfxsurn . '</span>', count($indis))
										  : $spfxsurn,
									  'weight' => count($indis),
									  'params' => array(
										  'url' => $surn ?
	...
	function print_events_table($startjd, $endjd, $events = 'BIRT MARR DEAT', $only_living = false, $sort_by = 'anniv')
	{
		...
		foreach ($filtered_events as $n => $fact) {
			...
			$html .= '<td>' . $fact->getDate()
								   ->Display(empty(Globals::i()->SEARCH_SPIDER)) . '</td>';
			$html .= '<td>' . $n . '</td>';
	 ...
	 *
	 * This performs the same function as FunctionsPrintLists::i()->print_events_table(), but formats the output differently.
	 *
	 * @param integer $startjd
	 * @param integer $endjd
	 * @param string  $events
	...
	function print_events_list($startjd, $endjd, $events = 'BIRT MARR DEAT', $only_living = false, $sort_by = 'anniv')
	{
		...
		foreach ($filtered_events as $fact) {
			...
			$html .= $fact->getLabel() . ' — ' . $fact->getDate()
													  ->Display(true);
			if ($fact->anniv) {
			...
			if (!$fact->getPlace()
				...
				$html .= ' — <a href="' . $fact->getPlace()
											   ->getURL() . '">' . $fact->getPlace()
																		->getFullName() . '</a>';
			}
			$html .= '</div>';
		}
	 ...
	 * @return string
	 */
	protected function print_chart_by_age($data, $title)
	{
		$count  = 0;
		$agemax = 0;
		$vmax   = 0;
	 ...
	 * @return string
	 */
	protected function print_chart_by_decade($data, $title)
	{
		$count = 0;
		$vmax  = 0;
		foreach ($data as $v) {
