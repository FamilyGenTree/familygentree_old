...
class FunctionsImport
{
        ...
        foreach ($matches as $n => $match) {
            ...
            switch ($tag) {
                ...
                case 'FILE':
                    ...
                    $GEDCOM_MEDIA_PATH = Globals::i()->WT_TREE->getPreference('GEDCOM_MEDIA_PATH');
                ...
                case 'CONC':
                    ...
                    $newrec .= (Globals::i()->WT_TREE->getPreference('WORD_WRAPPED_NOTES') ? ' ' : '') . $data;
                    break;
            }
        }
    ...
    function import_record($gedrec, $ged_id, $update)
    {
        ...
        $gedrec = $this->reformat_record_import($gedrec);

        // import different types of records
        if (preg_match('/^0 @(' . WT_REGEX_XREF . ')@ (' . WT_REGEX_TAG . ')/', $gedrec, $match)) {
            list(, $xref, $type) = $match;
            // check for a _UID, if the record doesn't have one, add one
            if (Globals::i()->WT_TREE->getPreference('GENERATE_UIDS') && !strpos($gedrec, "\n1 _UID ")) {
        ...
        switch ($type) {
            case 'INDI':
                ...
                $gedrec = $this->convert_inline_media($ged_id, $gedrec);

                $record = new Individual($xref, $gedrec, null, $ged_id);
                if (Globals::i()->WT_TREE->getPreference('USE_RIN') && preg_match('/\n1 RIN (.+)/', $gedrec, $match)) {
                    $rin = $match[1];
                } else {
                    $rin = $xref;
                }
                $sql_insert_indi->execute(array(
                                              $xref,
                                              $ged_id,
                                              $rin,
                                              $record->getSex(),
                ...
                $this->update_places($xref, $ged_id, $gedrec);
                $this->update_dates($xref, $ged_id, $gedrec);
                $this->update_links($xref, $ged_id, $gedrec);
                $this->update_names($xref, $ged_id, $record);
                break;
            case 'FAM':
                // Convert inline media into media objects
                $gedrec = $this->convert_inline_media($ged_id, $gedrec);
            ...
            case 'REPO':
                ...
                $sql_insert_other->execute(array(
                                               $xref,
                                               $ged_id,
                                               $type,
                                               $gedrec
            ...
            case 'OBJE':
                ...
                $sql_insert_media->execute(array(
                                               $xref,
                                               $record->extension(),
                                               $record->getMediaType(),
                                               $record->title,
            ...
            foreach ($secalp as $place) {
                ...
                if (isset($placecache[$key])) {
                    ...
                    if (!isset($personplace[$key])) {
                        $personplace[$key] = 1;
                        $sql_insert_placelinks->execute(array(
                                                            $parent_id,
                                                            $gid,
                                                            $ged_id
                                                        ));
                ...
                if ($search) {
                    ...
                    $tmp = $sql_select_places->execute(array(
                                                           $ged_id,
                                                           $parent_id,
                                                           $place
                                                       ))
                ...
                if (!$search) {
                    ...
                    $sql_insert_places->execute(array(
                                                    $place,
                                                    $parent_id,
                                                    $ged_id,
                                                    $std_soundex,
    ...
    function update_names($xref, $ged_id, GedcomRecord $record)
    {
        ...
        foreach ($record->getAllNames() as $n => $name) {
            if ($record instanceof Individual) {
                ...
                $sql_insert_name_indi->execute(array(
                                                   $ged_id,
                                                   $xref,
                                                   $n,
                                                   $name['type'],
                ...
                $sql_insert_name_other->execute(array(
                                                    $ged_id,
                                                    $xref,
                                                    $n,
                                                    $name['type'],
    ...
    {
        while (preg_match('/\n1 OBJE(?:\n[2-9].+)+/', $gedrec, $match)) {
            $gedrec = str_replace($match[0], $this->create_media_object(1, $match[0], $ged_id), $gedrec);
        }
        while (preg_match('/\n2 OBJE(?:\n[3-9].+)+/', $gedrec, $match)) {
            $gedrec = str_replace($match[0], $this->create_media_object(2, $match[0], $ged_id), $gedrec);
        }
        while (preg_match('/\n3 OBJE(?:\n[4-9].+)+/', $gedrec, $match)) {
            $gedrec = str_replace($match[0], $this->create_media_object(3, $match[0], $ged_id), $gedrec);
        }
    ...
    function create_media_object($level, $gedrec, $ged_id)
    {
        ...
        $xref = $sql_select_media->execute(array(
                                               $file,
                                               $titl,
                                               $ged_id
                                           ))
    ...
    function accept_all_changes($xref, $ged_id)
    {
        ...
        foreach ($changes as $change) {
            if (empty($change->new_gedcom)) {
                // delete
                $this->update_record($change->old_gedcom, $ged_id, true);
            } else {
                // add/update
    ...
    function update_record($gedrec, $ged_id, $delete)
    {
        ...
        if (!$delete) {
            $this->import_record($gedrec, $ged_id, true);
        }
    }
}