...
class FunctionsPrintFacts
{
    ...
    function print_fact(Fact $fact, GedcomRecord $record)
    {
        ...
        switch ($fact->getTag()) {
            case 'NOTE':
                $this->print_main_notes($fact, 1);

                return;
            case 'SOUR':
                $this->print_main_sources($fact, 1);

                return;
            case 'OBJE':
                $this->print_main_media($fact, 1);
        ...
        switch ($fact->getTag()) {
            ...
            case 'REPO':
                if (preg_match('/^@(' . WT_REGEX_XREF . ')@$/', $fact->getValue(), $match)) {
                    $this->print_repository_record($match[1]);
                } else {
        ...
        if ($type) {
            ...
            echo WT_Gedcom_Tag::getLabelValue('TYPE', Filter::escapeHtml($type));
        }
            ...
            switch ($match[1]) {
                ...
                case 'EVEN': // 0 SOUR / 1 DATA / 2 EVEN / 3 DATE / 3 PLAC
                    ...
                    if (count($events) == 1) {
                        ...
                        echo WT_Gedcom_Tag::getLabelValue('EVEN', implode(I18N::$list_separator, $events));
                    }
                ...
                case '_WT_USER':
                    ...
                    if ($user) {
                        echo WT_Gedcom_Tag::getLabelValue('_WT_USER', Filter::escapeHtml($user->getRealName()));
                    } else {
                        echo WT_Gedcom_Tag::getLabelValue('_WT_USER', Filter::escapeHtml($match[2]));
                    }
                    break;
                case 'RESN':
                    switch ($match[2]) {
                        case 'none':
                            ...
                            echo WT_Gedcom_Tag::getLabelValue('RESN', '<i class="icon-resn-none"></i> ' . I18N::translate('Show to visitors'));
                            break;
                        case 'privacy':
                            echo WT_Gedcom_Tag::getLabelValue('RESN', '<i class="icon-resn-privacy"></i> ' . I18N::translate('Show to members'));
                            break;
                        case 'confidential':
                            echo WT_Gedcom_Tag::getLabelValue('RESN', '<i class="icon-resn-confidential"></i> ' . I18N::translate('Show to managers'));
                            break;
                        case 'locked':
                            echo WT_Gedcom_Tag::getLabelValue('RESN', '<i class="icon-resn-locked"></i> ' . I18N::translate('Only managers can edit'));
                            break;
                        default:
                            echo WT_Gedcom_Tag::getLabelValue('RESN', Filter::escapeHtml($match[2]));
                            break;
                    }
                    break;
                case 'CALN':
                    echo WT_Gedcom_Tag::getLabelValue('CALN', Filter::expandUrls($match[2]));
                    break;
                    ...
                    if (!Globals::i()->WT_TREE->getPreference('HIDE_GEDCOM_ERRORS') || WT_Gedcom_Tag::isTag($match[1])) {
                        if (preg_match('/^@(' . WT_REGEX_XREF . ')@$/', $match[2], $xmatch)) {
                            ...
                            if ($linked_record) {
                                ...
                                echo WT_Gedcom_Tag::getLabelValue($fact->getTag() . ':' . $match[1], Filter::escapeHtml($match[2]));
                            }
                        } else {
                            // Non links
                            echo WT_Gedcom_Tag::getLabelValue($fact->getTag() . ':' . $match[1], Filter::escapeHtml($match[2]));
        ...
        echo $this->print_fact_sources($fact->getGedcom(), 2);
        echo FunctionsPrint::i()->print_fact_notes($fact->getGedcom(), 2);
        $this->print_media_links($fact->getGedcom(), 2);
        echo '</td></tr>';
    }
     ...
     *
     * this function is called by the $this->print_fact function and other functions to
        ...
        for ($j = 0; $j < $ct; $j++) {
            ...
            if ($source) {
                if ($source->canShow()) {
                    ...
                    $data .= $this->printSourceStructure($this->getSourceStructure($srec));
                    $data .= '<div class="indent">';
                    ob_start();
                    $this->print_media_links($srec, $nlevel);
                    $data .= ob_get_clean();
        ...
        while ($objectNum < count($omatch)) {
            ...
            if ($media) {
                if ($media->canShow()) {
                    ...
                    echo $this->print_fact_sources($media->getGedcom(), $nlevel);
        ...
        for ($j = 0; $j < $ct; $j++) {
            ...
            if (!$source || $source->canShow()) {
                ...
                if ($source) {
                    ...
                    echo $this->printSourceStructure($this->getSourceStructure($srec));
                    echo '<div class="indent">';
                    $this->print_media_links($srec, $nlevel);
                    if ($nlevel == 2) {
                        $this->print_media_links($source->getGedcom(), 1);
                    }
     ...
     *  $this->getSourceStructure() function.
     *
     * @param string[] $textSOUR
     *
     * @return string
     */
    protected function printSourceStructure($textSOUR)
    {
        $html = '';

        if ($textSOUR['PAGE']) {
            $html .= WT_Gedcom_Tag::getLabelValue('PAGE', Filter::expandUrls($textSOUR['PAGE']));
        }

        if ($textSOUR['EVEN']) {
            $html .= WT_Gedcom_Tag::getLabelValue('EVEN', Filter::escapeHtml($textSOUR['EVEN']));
            if ($textSOUR['ROLE']) {
                $html .= WT_Gedcom_Tag::getLabelValue('ROLE', Filter::escapeHtml($textSOUR['ROLE']));
            }
        }
     ...
     * @return string[]
     */
    protected function getSourceStructure($srec)
    {
    ...
    function print_main_notes(Fact $fact, $level)
    {
        ...
        for ($j = 0; $j < $ct; $j++) {
            ...
            if (!empty($noterec)) {
                echo $this->print_fact_sources($noterec, 1);
            }
            ...
            if (!$media || $media->canShow()) {
                ...
                if ($media) {
                    ...
                    switch ($media->isPrimary()) {
                        case 'Y':
                            echo WT_Gedcom_Tag::getLabelValue('_PRIM', I18N::translate('yes'));
                            break;
                        case 'N':
                    ...
                    echo FunctionsPrint::i()->print_fact_notes($media->getGedcom(), 1);
                    echo $this->print_fact_sources($media->getGedcom(), 1);
                } else {
                    echo $xref;
                }
                echo '</td></tr>';
