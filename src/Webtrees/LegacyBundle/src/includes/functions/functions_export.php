...
class FunctionsExport
{
    ...
    function gedcom_header($gedfile)
    {
        ...
        $SUBN = "";
        $SUBM = "\n1 SUBM @SUBM@\n0 @SUBM@ SUBM\n1 NAME " . Auth::user()
                                                                ->getUserName(); // The SUBM record is mandatory
     ...
     *                                'path':         what constant should prefix all media file paths?  (eg: media/  or
     *                                c:\my pictures\my family
     *                                'slashes':      what folder separators apply to media file paths?  (forward,
     *                                backward)
     *
     */
    function export_gedcom($gedcom, $gedout, $exportOptions)
    {
        ...
        $head = $this->gedcom_header($gedcom);
        if ($exportOptions['toANSI'] == 'yes') {
            $head = str_replace('UTF-8', 'ANSI', $head);
            $head = utf8_decode($head);
        }
        $head = $this->reformat_record_export($head);
        fwrite($gedout, $head);
        ...
        $rows       = Database::prepare(
            ...
            " FROM `##media` WHERE m_file=? ORDER BY m_id"
                              ...
                              ->fetchAll();
        foreach ($rows as $row) {
            $rec = Media::getInstance($row->xref, $row->gedcom_id, $row->gedcom)
                        ->privatizeGedcom($access_level);
            $rec = $this->convert_media_path($rec, $exportOptions['path']);
            if ($exportOptions['toANSI'] == 'yes') {
                $rec = utf8_decode($rec);
            }
        ...
        $rows = Database::prepare(
            ...
            " FROM `##sources` WHERE s_file=? ORDER BY s_id"
                        ...
                        ->fetchAll();
        foreach ($rows as $row) {
            $rec = Source::getInstance($row->xref, $row->gedcom_id, $row->gedcom)
                         ->privatizeGedcom($access_level);
            if ($exportOptions['toANSI'] == 'yes') {
                $rec = utf8_decode($rec);
            }
        ...
        $rows = Database::prepare(
            ...
            " FROM `##individuals` WHERE i_file=? ORDER BY i_id"
                        ...
                        ->fetchAll();
        foreach ($rows as $row) {
            $rec = Individual::getInstance($row->xref, $row->gedcom_id, $row->gedcom)
                             ->privatizeGedcom($access_level);
            if ($exportOptions['toANSI'] == 'yes') {
                $rec = utf8_decode($rec);
            }
            $buffer .= $this->reformat_record_export($rec);
        ...
        $rows = Database::prepare(
            ...
            " FROM `##families` WHERE f_file=? ORDER BY f_id"
                        ...
                        ->fetchAll();
        foreach ($rows as $row) {
            $rec = Family::getInstance($row->xref, $row->gedcom_id, $row->gedcom)
                         ->privatizeGedcom($access_level);
            if ($exportOptions['toANSI'] == 'yes') {
                $rec = utf8_decode($rec);
            }
