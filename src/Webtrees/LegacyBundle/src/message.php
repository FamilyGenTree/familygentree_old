...
if (Auth::check()) {
    ...
    if (preg_match('/(?!' . preg_quote(Config::get(Config::BASE_URL), '/') . ')(((?:ftp|http|https):\/\/)[a-zA-Z0-9.-]+)/', $subject . $body, $match)) {
        $errors .=
            ...
            I18N::translate('You should delete the “%1$s” from “%2$s” and try again.', $match[2], $match[1]) . '</p>' .
    ...
    if ($message['method'] !== 'messaging3' && $message['method'] !== 'mailto' && $message['method'] !== 'none') {
        Database::i()->prepare("INSERT INTO `##message` (sender, ip_address, user_id, subject, body) VALUES (? ,? ,? ,? ,?)")
                ->execute(array(
                              ...
                              // Remove the <br> that we added for the external email.  TODO: create different messages
                          ));
    }
    if ($message['method'] !== 'messaging') {
        if ($sender) {
