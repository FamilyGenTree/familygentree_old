...
$lock_file_text = I18N::translate('This website is being upgraded.  Try again in a few minutes.') . PHP_EOL . FunctionsDate::i()->format_timestamp(WT_TIMESTAMP) . I18N::translate('UTC');
...
if ($continue) {
    ...
    echo '<p>', I18N::translate('It can take several minutes to download and install the upgrade.  Be patient.'), '</p>';
    echo '<button type="submit" name="continue" value="1">', /* I18N: %s is a version number, such as 1.2.3 */
    I18N::translate('Upgrade to webtrees %s.', $latest_version_html), '</button>';
    echo '</form>';
...
echo '<li>', /* I18N: The system is about to [...] */
I18N::translate('Check for custom modules…');
...
if ($custom_modules) {
    ...
    echo '<br>', '<button type="submit" name="modules" value="ignore">', /* I18N: Ignore the warnings, and [...] */
    I18N::translate('Upgrade anyway'), '</button> — ', I18N::translate('Caution: old modules may not work, or they may prevent webtrees from working.');
    echo '</li></ul></form>';
...
echo '<li>', /* I18N: The system is about to [...] */
I18N::translate('Check for custom themes…');
...
echo '<li>', /* I18N: The system is about to [...] */
I18N::translate('Export all family trees to GEDCOM files…');
...
echo '<br>', /* I18N: %1$s is a number of KB, %2$s is a (fractional) number of seconds */
I18N::translate('%1$s KB were downloaded in %2$s seconds.', I18N::number($zip_size / 1024), I18N::number($end_time - $start_time, 2));
if ($zip_size) {
    echo $icon_success;
} else {
    echo $icon_failure;
    ...
    if (preg_match('/^https:/', $download_url) && !in_array('ssl', stream_get_transports())) {
        echo '<br>', /* I18N: http://en.wikipedia.org/wiki/Https */
        I18N::translate('This server does not support secure downloads using HTTPS.');
    }
}
...
echo '<li>', /* I18N: The system is about to [...]; %s is a .ZIP file. */
I18N::translate('Unzip %s to a temporary folder…', basename($download_url));
...
if (is_array($res)) {
    ...
    echo '<br>', /* I18N: [...] from the .ZIP file, %2$s is a (fractional) number of seconds */
    I18N::plural('%1$s file was extracted in %2$s seconds.', '%1$s files were extracted in %2$s seconds.', count($res), count($res), I18N::number($end_time - $start_time, 2)), $icon_success;
} else {
    echo '<br>', I18N::translate('An error occurred when unzipping the file.'), $icon_failure;
...
echo '<li>', /* I18N: The system is about to [...] */
I18N::translate('Delete temporary files…');
