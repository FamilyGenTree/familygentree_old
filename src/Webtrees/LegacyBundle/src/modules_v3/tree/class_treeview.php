...
class TreeView
{
    ...
    public function drawViewport(Individual $root_person, $generations)
    {
        ...
        return array(
            $html,
            'var ' . $this->name . 'Handler = new TreeViewHandler("' . $this->name . '");'
        );
    }
    ...
    private function drawChildren(array $familyList, $gen = 1, $ajax = false)
    {
        ...
        if ($tc) {
            ...
            if (!$ajax) {
                $html = '<td align="right"' . ($gen == 0 ? ' abbr="c' . $f2load . '"'
                        : '') . '>' . $html . '</td>' . $this->drawHorizontalLine();
            }
        }
    ...
    private function drawPerson(Individual $person, $gen, $state, Family $pfamily = null, $order = null, $isRoot = false)
    {
        ...
        $html .= '<td' . ($isRoot ? ' style="width:1px"' : '') . '><div class="tv_box' . ($isRoot ? ' rootPerson'
                : '') . '" dir="' . Globals::i()->TEXT_DIRECTION . '" style="text-align: ' . (Globals::i()->TEXT_DIRECTION == "rtl"
                ? "right"
                : "left") . '; direction: ' . Globals::i()->TEXT_DIRECTION . '" abbr="' . $person->getXref() . '" onclick="' . $this->name . 'Handler.expandBox(this, event);">';
        ...
        if ($state >= 0 && (!empty($parent) || count($fop))) {
            ...
            if (count($fop)) {
                ...
                foreach ($fop as $p) {
                    ...
                    $html .= '<tr><td ' . ($gen == 0 ? ' abbr="p' . $p[1]->getXref() . '@' . $u . '"'
                            : '') . '>' . $this->drawPerson($p[0], $gen - 1, 1, $p[1], $u) . '</td></tr>';
                }
            }
            $html .= '</tbody></table></td>';
        }
