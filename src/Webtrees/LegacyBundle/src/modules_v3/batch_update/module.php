...
class batch_update_WT_Module extends Module implements ModuleConfigInterface
{
    ...
    function main()
    {
            ...
            switch ($this->action) {
                case 'update':
                    ...
                    if ($this->PLUGIN->doesRecordNeedUpdate($this->xref, $record)) {
                        ...
                        if ($newrecord != $record) {
                            if ($newrecord) {
                                GedcomRecord::getInstance($this->xref)
                                            ->updateRecord($newrecord, $this->PLUGIN->chan);
                            } else {
                                GedcomRecord::getInstance($this->xref)
                                            ->deleteRecord();
                            }
        ...
        foreach ($this->plugins as $class => $plugin) {
            $html .= '<option value="' . $class . '" ' . ($this->plugin == $class ? 'selected'
                    : '') . '>' . $plugin->getName() . '</option>';
        }
        $html .= '</select>';
        if ($this->PLUGIN) {
    ...
    public static function getLatestRecord($xref, $type)
    {
        switch ($type) {
            case 'INDI':
                return Individual::getInstance($xref)
                                 ->getGedcom();
            case 'FAM':
                return Family::getInstance($xref)
                             ->getGedcom();
            case 'SOUR':
                return Source::getInstance($xref)
                             ->getGedcom();
            case 'REPO':
                return Repository::getInstance($xref)
                                 ->getGedcom();
            case 'OBJE':
                return Media::getInstance($xref)
                            ->getGedcom();
            case 'NOTE':
                return Note::getInstance($xref)
                           ->getGedcom();
            default:
