 ...
 */
class personal_facts_WT_Module extends Module implements ModuleTabInterface
{
    /** {@inheritdoc} */
    ...
    public function getTabContent()
    {
        ...
        foreach ($controller->record->getFacts() as $fact) {
            switch ($fact->getTag()) {
                ...
                case 'FAMS':
                    break;
                default:
                    if (!array_key_exists('extra_info', Module::getActiveSidebars()) || !extra_info_WT_Module::showFact($fact)) {
                        $indifacts[] = $fact;
                    }
                    break;
            }
        ...
        echo $controller->record->getTree()
                                ->getPreference('EXPAND_RELATIVES_EVENTS') ? 'checked' : '';
    ...
    {
        $SHOW_RELATIVES_EVENTS = $individual->getTree()
            ...
            if (strstr($SHOW_RELATIVES_EVENTS, '_MARR_PARE')) {
                ...
                foreach ($person->getChildFamilies() as $sfamily) {
                    foreach ($sfamily->getFacts(WT_EVENTS_MARR) as $fact) {
                        if ($fact->getDate()
                                 ->isOK()
                            && Date::Compare($birt_date, $fact->getDate()) <= 0
                            && Date::Compare($fact->getDate(), $deat_date) <= 0
                        ) {
