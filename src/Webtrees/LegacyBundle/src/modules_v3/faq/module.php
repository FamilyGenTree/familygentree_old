 ...
 * Class faq_WT_Module
 */
class faq_WT_Module extends Module implements ModuleMenuInterface, ModuleConfigInterface
{
    /** {@inheritdoc} */
    ...
    private function edit()
    {
        if (Filter::postBool('save') && Filter::checkCsrf()) {
            ...
            <ol class="breadcrumb small">
                ...
                <li>
                    <a href="module.php?mod=<?php echo $this->getName(); ?>&mod_action=admin_config"><?php echo I18N::translate('Frequently asked questions'); ?></a>
                </li>
    ...
    private function show()
    {
        ...
        foreach ($faqs as $id => $faq) {
            if (!$faq->languages || in_array(WT_LOCALE, explode(',', $faq->languages))) {
                ...
                echo '</div>';
                echo '</div>';
                echo '<div class="faq_body">', substr($faq->body, 0, 1) == '<' ? $faq->body
                    : nl2br($faq->body, false), '</div>';
                echo '<hr>';
            }
        }
    }
    ...
    private function config()
    {
        ...
        if (empty($faqs)) {
            ...
            foreach ($faqs as $faq) {
                ...
                echo '<div class="faq_edit_content">', substr($faq->faqbody, 0, 1) == '<' ? $faq->faqbody
                    : nl2br($faq->faqbody, false), '</div></div></td></tr>';
            }
            echo '</table>';
        }
    }
