 ...
 */
class todo_WT_Module extends Module implements ModuleBlockInterface
{
    /** {@inheritdoc} */
    ...
    public function getBlock($block_id, $template = true, $cfg = null)
    {
        ...
        foreach (FunctionsDbPhp::i()->get_calendar_events(0, $end_jd, '_TODO', WT_GED_ID) as $fact) {
            ...
            if ($user_name === Auth::user()
                                   ->getUserName()
                || !$user_name && $show_unassigned
                || $user_name && $show_other
            ) {
                $content .= '<tr>';
                //-- Event date (sortable)
                $content .= '<td>'; //hidden by datables code
                $content .= $fact->getDate()
                                 ->JD();
                $content .= '</td>';
                $content .= '<td class="wrap">' . $fact->getDate()
