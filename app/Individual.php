...
class Individual extends GedcomRecord
{
    ...
    public function getLifeSpan()
    {
        return
            ...
            I18N::translate(
                '%1$s–%2$s',
                '<span title="' . strip_tags($this->getBirthDate()
                                                  ->display()) . '">' . $this->getBirthDate()
                                                                             ->MinDate()
                                                                             ->format('%Y') . '</span>',
                '<span title="' . strip_tags($this->getDeathDate()
                                                  ->display()) . '">' . $this->getDeathDate()
                                                                             ->MinDate()
                                                                             ->format('%Y') . '</span>'
            );
    }
    ...
    function getEstimatedDeathDate()
    {
        if ($this->_getEstimatedDeathDate === null) {
            ...
            if ($this->_getEstimatedDeathDate === null) {
                if ($this->getEstimatedBirthDate()
                         ->MinJD()
                ) {
                    $this->_getEstimatedDeathDate = $this->getEstimatedBirthDate()
                                                         ->AddYears($this->tree->getPreference('MAX_ALIVE_AGE'), 'BEF');
                } else {
    ...
    function getSpouseStepFamilies()
    {
        ...
        foreach ($families as $family) {
            $spouse = $family->getSpouse($this);
            if ($spouse) {
                foreach ($family->getSpouse($this)
                                ->getSpouseFamilies() as $step_family) {
                    if (!in_array($step_family, $families, true)) {
                        $step_families[] = $step_family;
                    }
                }
     ...
     * @return string
     */
    function getStepFamilyLabel(Family $step_family)
    {
        foreach ($this->getChildFamilies() as $family) {
                            ...
                            if ($parent->getSex() == 'M') {
                                ...
                                if ($step_family->getSpouse($step_parent)) {
                                    return
                                        ...
                                        I18N::translate('Father’s family with %s', $step_family->getSpouse($step_parent)
                                                                                               ->getFullName());
                                } else {
                                    return
                                        /* I18N: A step-family. */
                                ...
                                if ($step_family->getSpouse($step_parent)) {
                                    return
                                        /* I18N: A step-family.  %s is an individual’s name */
                                        I18N::translate('Mother’s family with %s', $step_family->getSpouse($step_parent)
                                                                                               ->getFullName());
                                } else {
                                    return
                                        /* I18N: A step-family. */
