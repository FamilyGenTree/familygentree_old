...
class Fact
{
    ...
    public function getTarget()
    {
        ...
        switch ($this->tag) {
            case 'FAMC':
            case 'FAMS':
                return Family::getInstance($xref, $this->getParent()
                                                       ->getTree()
                                                       ->getTreeId());
            case 'HUSB':
            case 'WIFE':
            case 'CHIL':
                return Individual::getInstance($xref, $this->getParent()
                                                           ->getTree()
                                                           ->getTreeId());
            case 'SOUR':
                return Source::getInstance($xref, $this->getParent()
                                                       ->getTree()
                                                       ->getTreeId());
            case 'OBJE':
                return Media::getInstance($xref, $this->getParent()
                                                      ->getTree()
                                                      ->getTreeId());
            case 'REPO':
                return Repository::getInstance($xref, $this->getParent()
                                                           ->getTree()
                                                           ->getTreeId());
            case 'NOTE':
                return Note::getInstance($xref, $this->getParent()
                                                     ->getTree()
                                                     ->getTreeId());
            default:
                return GedcomRecord::getInstance($xref, $this->getParent()
                                                             ->getTree()
                                                             ->getTreeId());
        }
    }
    ...
    public function canShow($access_level = WT_USER_ACCESS_LEVEL)
    {
        ...
        $fact_privacy            = $this->parent->getTree()
                                                ->getFactPrivacy();
        $individual_fact_privacy = $this->parent->getTree()
    ...
    public function canEdit()
    {
        ...
        return
            ...
            && (
                WT_USER_GEDCOM_ADMIN || WT_USER_CAN_EDIT && strpos($this->gedcom, "\n2 RESN locked") === false && $this->getTag() != 'RESN' && $this->getTag() != 'CHAN'
            );
    }
    ...
    {
        if ($this->place === null) {
            $this->place = new Place($this->getAttribute('PLAC'), $this->getParent()
                                                                       ->getTree()
                                                                       ->getTreeId());
        }
    ...
    public function getCitations()
    {
        ...
        foreach ($matches as $match) {
            $source = Source::getInstance($match[2], $this->getParent()
                                                          ->getTree()
                                                          ->getTreeId());
            if ($source->canShow()) {
                $citations[] = $match[1];
    ...
    public function getNotes()
    {
        ...
        foreach ($matches[1] as $match) {
            ...
            if (preg_match('/@(' . WT_REGEX_XREF . ')@/', $note, $nmatch)) {
                $note = Note::getInstance($nmatch[1], $this->getParent()
                                                           ->getTree()
                                                           ->getTreeId());
    ...
    public function getMedia()
    {
        ...
        foreach ($matches[1] as $match) {
            $obje = Media::getInstance($match, $this->getParent()
                                                    ->getTree()
                                                    ->getTreeId());
            if ($obje->canShow()) {
                $media[] = $obje;
    ...
    public function summary()
    {
        ...
        if ($target) {
            ...
            $date = $this->getDate();
            if ($this->getTag() == 'BIRT' && $this->getParent() instanceof Individual
                && $this->getParent()
                        ->getTree()
        ...
        if (empty($factsort)) {
            $factsort = array_flip(
                array(
                    ...
                    '_DEAT_',
                    '_BURI_',
                    // other events of close relatives
                    'CENS',
                    'OCCU',
                    'RESI',
                    'PROP',
